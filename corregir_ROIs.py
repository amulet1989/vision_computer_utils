...
good_matches = []
for m, n in matches:
    if m.distance < 0.7 * n.distance:  # 0.75
...
roi_original = np.float32([[[337, 125], [415, 125], [457, 456], [290, 460]]])
# 52 [[337, 125], [415, 125], [457, 456], [290, 460]]
# 62 [[27, 258], [194, 266], [309, 339], [307, 477], [20, 476]]
roi_transformada = cv2.perspectiveTransform(roi_original, M)

# roi_transformada = np.floor(np.array(roi_transformada)).astype(np.int16)[0]
# roi_original = np.floor(np.array(roi_original)).astype(np.int16)[0]
...
# cv2.polylines(imagen1, [np.int32(roi_original)], True, (0, 255, 0), 2)
# Dibujar ROI transformada en imagen2
# cv2.polylines(imagen2, [np.int32(roi_transformada)], True, (0, 255, 0), 2)
